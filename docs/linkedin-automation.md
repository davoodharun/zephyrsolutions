...
7. **Company page** (optional): To publish to a company page you need both:
   ...
   - **Token with org scopes**: For company page, the workflow **prefers the refresh-token flow** when `LINKEDIN_REFRESH_TOKEN`, `LINKEDIN_CLIENT_ID`, and `LINKEDIN_CLIENT_SECRET` are set—it exchanges the refresh token for a fresh access token each run (avoids 401 from stale or mis-pasted tokens). Get a token with `LINKEDIN_USE_COMPANY_PAGE=true`, then set **`LINKEDIN_REFRESH_TOKEN`** (and client id/secret) in Secrets. You can leave `LINKEDIN_ACCESS_TOKEN` unset so the workflow always uses the refresh flow. Without `w_organization_social` and `rw_ads` you may get **400 "Organization permissions must be used"** or **401 Unauthorized**.
