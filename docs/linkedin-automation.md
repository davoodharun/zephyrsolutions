...
7. **Company page** (optional): To publish to a company page you need both:
   ...
   - **Token with org scopes**: For company page, the workflow **prefers the refresh-token flow** when `LINKEDIN_REFRESH_TOKEN`, `LINKEDIN_CLIENT_ID`, and `LINKEDIN_CLIENT_SECRET` are set. Get a token with `LINKEDIN_USE_COMPANY_PAGE=true`, then set **`LINKEDIN_REFRESH_TOKEN`** (and client id/secret) in Secrets. You can leave `LINKEDIN_ACCESS_TOKEN` unset. If you see **401 on token exchange** ("LinkedIn token exchange failed"): the refresh token may be expired (LinkedIn refresh tokens last ~1 year) or revoked—run the get-token script again and update `LINKEDIN_REFRESH_TOKEN`. Ensure **redirect_uri** matches: the workflow uses `vars.LINKEDIN_REDIRECT_URI` or `http://localhost:8080/callback`; it must match exactly what you used when obtaining the refresh token. Paste the full refresh token into the secret with no leading/trailing newlines.
